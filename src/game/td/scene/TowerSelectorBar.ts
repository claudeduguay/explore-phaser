  ...
  addTowerButton(selector: TowerSelector, model: ITowerModel, x: number, y: number) {
    ...
    button.onClick = () => {
      ...
      if (model.locked) {
        ...
        if (this.lockIcon) {
          button.remove(this.lockIcon)
        }
        if (this.lockText) {
          button.remove(this.lockText)
        }
      }
    ...
    // )
    if (model.locked) {
      this.lockIcon = this.scene.add.icon(0, -14, 0xe897, {
        ...DEFAULT_STYLE,
        fontSize: 42,
        color: "orange",
        shadow: {
      ...
      this.lockText = this.scene.add.text(0, 0, `$${model.stats.cost}`, {
        fontFamily: DEFAULT_FONT_FAMILY,
        fontSize: 18,
        color: "orange",
        shadow: {
      ...
      button.add(this.lockIcon)
      button.add(this.lockText)
    }
